...
clf = KNeighborsClassifier(n_neighbors=15)  # Adjust n_neighbors as needed
clf.fit(X_train, y_train)
y_pred_knn = clf.predict(X_test)
acc_knn = round(clf.score(X_test, y_test) * 100, 2)  # Calculate accuracy on the test set
print(str(acc_knn) + '%')
...
y_pred = (y_pred > 0.5)
acc_ANN = round((classifier.evaluate(X_test, y_test)[1]) * 100, 2)  # Calculate accuracy on the test set using evaluate
print(str(acc_ANN) + '%')
